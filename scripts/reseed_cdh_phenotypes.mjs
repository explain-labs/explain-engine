// Re-seed the CDH phenotype scenarios to their calibrated steady state (generalises
// reseed_term_neonate_cdh.mjs over the cdh_* family). All calibration already lives in each scenario JSON
// (written by scripts/_make_cdh_phenotypes.mjs), so this just warms the model to steady state and
// serializes it back into model_definition the way the app's save-state does (Model._processModelState) —
// baking the equilibrium gas/volume seeds and clearing startup transients so the file loads at its
// operating point.
//
//   node scripts/reseed_cdh_phenotypes.mjs <key|--all> [--seconds 250] [--write]

import fs from "node:fs";
import { register } from "node:module";
register("./resolve-extensionless.mjs", import.meta.url);

const KEYS = ["cdh_severe", "cdh_moderate", "cdh_lv_dysfunction"];
const argv = process.argv.slice(2);
const secIdx = argv.indexOf("--seconds");
const SECONDS = secIdx >= 0 ? Number(argv[secIdx + 1]) : 250;
...
if (keys.length === 0 || keys.some((k) => !KEYS.includes(k))) {
  console.error(`usage: node scripts/reseed_cdh_phenotypes.mjs <key|--all> [--seconds N] [--write]\nkeys: ${KEYS.join(", ")}`);
  process.exit(1);
}
