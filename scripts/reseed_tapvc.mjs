...
// calibration lives in the scenario JSON (written by scripts/_make_tapvc.mjs); this warms the model to
...
//
//   node scripts/reseed_tapvc.mjs <key|--all> [--seconds 300] [--write]

import fs from "node:fs";
import { register } from "node:module";
register("./resolve-extensionless.mjs", import.meta.url);

const KEYS = ["tapvc","tapvc_obstructed"];
...
if (keys.length === 0 || keys.some((k) => !KEYS.includes(k))) {
  console.error(`usage: node scripts/reseed_tapvc.mjs <key|--all> [--seconds N] [--write]\nkeys: ${KEYS.join(", ")}`);
  process.exit(1);
}
