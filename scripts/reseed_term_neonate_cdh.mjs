// Re-seed term_neonate_cdh.json to its calibrated steady state (same approach as reseed_term_fetus.mjs).
//
// All CDH calibration already lives in the scenario JSON (written by scripts/_make_term_neonate_cdh.mjs),
...
//
//   node scripts/reseed_term_neonate_cdh.mjs [--seconds 250] [--write]
...
const file = new URL("../public/model_definitions/term_neonate_cdh.json", import.meta.url);
...
// CDH calibration is fully baked into the JSON; no extra deltas here — just warm to steady state.
...
else { fs.writeFileSync("/tmp/term_neonate_cdh_reseed.json", out); console.log("dry run -> /tmp/term_neonate_cdh_reseed.json (pass --write to commit)"); }
