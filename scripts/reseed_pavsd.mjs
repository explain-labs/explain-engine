...
// calibration lives in the scenario JSON (written by scripts/_make_pavsd.mjs); this warms the model to
...
//
//   node scripts/reseed_pavsd.mjs <key|--all> [--seconds 300] [--write]
...
if (keys.length === 0 || keys.some((k) => !KEYS.includes(k))) {
  console.error(`usage: node scripts/reseed_pavsd.mjs <key|--all> [--seconds N] [--write]\nkeys: ${KEYS.join(", ")}`);
  process.exit(1);
}
